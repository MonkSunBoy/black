...
def usage():
	...
	print "-e --excute=file_to_run       - execute the given file ipon receiving a connection"
...
def main():
	...
	# upload things, execute commands, and drops a shell back
...
def server_loop():
	...
	while True:
		...
		client_thread = threading.Thread(target=client_hanlder,
			args = (client_socket,))
		client_thread.start()
...
def client_hanlder(client_socket):
	...
	if len(upload_destination):
		...
		# keep reading data util none is avaliable
		while True:
	...
	if command:
		while True:
			...
			# now we receive util we see a linefeed (enter key)
			cmd_buffer = ""
			while "\n" not in cmd_buffer:
